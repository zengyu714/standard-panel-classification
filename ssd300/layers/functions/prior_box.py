    ...
    def forward(self):
        ...
        if self.version == 'v2':
            for k, f in enumerate(self.feature_maps):
                # inner product: f x f boxes, (i, j) is the coordinate
                for i, j in product(range(f), repeat=2):
                    f_k = self.image_size / self.steps[k]  # size of the k-th square feature map
                    # **unit** center x,y
                    cx = (j + 0.5) / f_k
                    cy = (i + 0.5) / f_k

                    # aspect_ratio: 1 (square shape)
                    # rel size: min_size
                    s_k = self.min_sizes[k] / self.image_size
                    mean += [cx, cy, s_k, s_k]

                    # aspect_ratio: 1 (square shape, bigger)
                    ...
                    for ar in self.aspect_ratios[i]:
                        if not (abs(ar - 1) < 1e-6):
                            ...
                            mean += [(c_x - c_w) / s_k, (c_y - c_h) / s_k,
                                     (c_x + c_w) / s_k, (c_y + c_h) / s_k]
        # back to torch land
        # len(output) = 34928 = 4 * 8732
        # 'aspect_ratios': [[2], [2, 3], [2, 3], [2, 3], [2], [2]] ==> [4, 6, 6, 6, 4, 4]
        # 4 * [4 * (38**2)+ 6 * (19**2)+ 6 * (10**2)+  6 * (5**2)+  4 * (3**2)+  4 * (1**2)]
