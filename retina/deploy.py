...
from helper.deploy import check_dir, DeployDataset
...
def save_deployment(dataset, save_dir, batch_size=64):
    ...
    for im_batch, p_batch in tqdm.tqdm(dataloader, total=total_num // batch_size, unit=' batch({})'.format(batch_size)):
        ...
        for i, (loc_pred, cls_pred) in enumerate(zip(loc_preds, cls_preds)):
            boxes, labels_idx = encoder.decode(loc_pred.cpu().data.squeeze(), cls_pred.cpu().data.squeeze(), (w, h))
            pred = []
            for box, label_idx in zip(boxes, labels_idx):
                pt = (box * scale).numpy()
...
def main():
    for cls in ['KLAC', 'KLFE', 'KLHC']:
        save_dir = 'deployment/retina/{}'.format(cls)
        check_dir(save_dir)
