...
# Retina Dataset Classes
# ---------------------------------------------------------------------------
class RetinaDataset(data.Dataset):
    def __init__(self, train_sets):
        self.means = (104, 117, 123)  # only support voc now
        self.input_size = 300
        if train_sets[0][1] == 'trainval':
            self.augment = Augmentation
        elif train_sets[0][1] == 'val':
            self.augment = BaseTransform
        else:
    ...
    def __getitem__(self, idx):
        im, gt, h, w = self.dataset.pull_item(idx)
        return im, gt
    ...
    def collate_fn(self, batch):
        ...
        for sample in batch:
            imgs.append(sample[0])
            loc = torch.Tensor(sample[1][:, :4])
            cls = torch.Tensor(sample[1][:, -1])
            loc_target, cls_target = self.data_encoder.encode(loc, cls, input_size=(w, h))
            loc_targets.append(loc_target)
            cls_targets.append(cls_target)
        return torch.stack(imgs), torch.stack(loc_targets), torch.stack(cls_targets)


def test_retina_dataset(phrase):
    import torchvision

    dataset = RetinaDataset(phrase)
